...
static int ice_devl_res_change(bool take, struct ice_adapter *adapter,
			       ...
			       void *owner)
{
	if (ice_is_devl_res_owned_by(adapter, res_id, owner) == take)
		return 0;
					   ...
					   struct ice_pf *pf)
{
	...
	if (ice_is_devl_res_owned_by(pf->adapter, ICE_RSS_LUT_PF, pf) &&
	    ice_is_devl_res_owned_by(pf->adapter, ICE_RSS_LUT_GLOBAL, pf))
		return 0;
				     ...
				     void *priv)
{
	...
	scoped_guard(ice_adapter_devl, pf->adapter) {
		...
		return DEVLINK_RESOURCE_SET_OCC_DONE;
	}
}
					 ...
					 void *priv)
{
	...
	scoped_guard(ice_adapter_devl, pf->adapter) {
		...
		err = ice_devl_res_change(size, pf->adapter, ICE_RSS_LUT_GLOBAL,
					  ICE_ANY_SLOT, pf);
		if (err)
			return err;
...
void ice_devlink_whole_dev_resources_register(struct device *pf_dev,
					      struct devlink *devlink)
{
	struct ice_adapter *adapter = devlink_priv(devlink);
	struct ice_pf *pf = dev_get_drvdata(pf_dev);
	int pf_lut_cnt = pf->hw.dev_caps.num_funcs;
	...
	ice_devl_res_register(devlink, adapter->resources, pf_dev);
	devl_unlock(devlink);
}
...
void ice_devl_pf_resources_register(struct ice_pf *pf)
{
	...
	ice_devl_res_register(devlink, pf_resources, ice_pf_to_dev(pf));
}
